<template>
  <div class="containerfu singerIN mt20">
    <div style="height:37px; width:100%;" class="music-people mt20">
      <h2 class="fleft"><span class="big">搜索</span>&ensp; <span class="small">结果</span></h2>
...
export default {
  ...
  methods:{
    ...
    songData () {
      this.$http({
        url:"/search?keywords= "+this.$route.params.name+"&type=1&limit=60",
      }).then(res =>{
        ...
        //数据分组用于分页显示
        let a = this.chunk(res.data.result.songs,15)
        console.log(a[3]);
    ...
    formatSongs(arr) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        obj.id = arr[i].id;
        obj.song = arr[i].name;
        obj.singer = arr[i].artists.name;
        obj.dt = this.formatDt(arr[i].duration); //由于返回的歌曲时长单位是ms，我们还要将其转换成00：00的格式
        b.push(obj)
      }
