  ...
  <div class="containerfu singerIN mt20">
    <div class="singer-title">
      <div class="fleft singer-img">
        <img :src="coverImgUrl+'?param=180y180'" class="img100" alt="">
      </div>
      <div class="fright singer-in">
        <h2>{{bangdanName}}</h2>
        <el-tooltip class="item" effect="light" transition="el-fade-in-linear" placement="bottom">
          <div slot="content" class="content-text" >{{bangdanDescription}}</div>
          <p class="tleft singer-desc">{{bangdanDescription}}</p>
        </el-tooltip>
...
export default {
  ...
  methods:{
    // singerData () {
    //   this.$http({
    //     url:"/api/artist/detail?id="+this.$route.params.id+"",
    //   }).then(res =>{
    ...
    songData () {
      this.$http({
        url:"/playlist/detail?id=" + this.$route.params.id + "&userid="+this.$store.state.userId+"",
      }).then(res =>{
        console.log(res.data.playlist);
        this.bangdanName = res.data.playlist.name
        this.bangdanDescription = res.data.playlist.description
        ...
        //数据分组用于分页显示
        let a = this.chunk(res.data.playlist.tracks,10)
